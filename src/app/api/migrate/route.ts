    ...
    if (error) {
      ...
      for (const statement of statements) {
        if (statement.trim()) {
          const { error: stmtError } = await supabase
            ...
            .select('*')
            .limit(1) // This will fail but we can catch the error
          
          // We'll handle this differently - let's provide manual instructions
        }
      }
      ...
      return NextResponse.json({ 
        ...
        instructions: [
          ...
          '3. Copy and paste the migration SQL from src/lib/database/schema-v2.sql',
