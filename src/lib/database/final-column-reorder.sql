-- Final migration to reorder database columns to exactly match the survey instrument order
...
CREATE TABLE IF NOT EXISTS survey_responses_backup_final_order AS SELECT * FROM survey_responses;
...
-- Create a simple survey_summary view
DO $$
BEGIN
    RAISE NOTICE 'Creating simplified survey_summary view...';
    ...
    RAISE NOTICE 'survey_summary view created successfully';
END $$;
...
BEGIN
    ...
    RAISE NOTICE '- Backup created: survey_responses_backup_final_order';
    RAISE NOTICE '';
...
BEGIN
    ...
    RAISE NOTICE 'Backup available in: survey_responses_backup_final_order';
END $$;
